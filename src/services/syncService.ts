...
import { DatabaseError, ValidationError, safeAsync } from '../types/database';
...
export class SyncService {
    ...
    return safeAsync(
      async () => {
        ...
        try {
          ...
          syncSession.status =
            syncSession.conflicts.length > 0 ? 'completed' : 'completed';
    ...
    return safeAsync(
      async () => {
        ...
        const lastSyncTimestamp =
          userResult.success && userResult.data?.updatedAt
      ...
      async () => {
        ...
        if (validateData) {
          const validationResult =
            await this.validateMigrationData(processedData);
        ...
        if (processedData.flashcards && processedData.flashcards.length > 0) {
          const flashcardsResult =
            await bulkOperationsService.bulkImportFlashcards(
              userId,
          ...
          if (flashcardsResult.success && flashcardsResult.data) {
            migrationResult.migratedItems.flashcards =
              flashcardsResult.data.insertedCount;
        ...
        if (
          ...
          processedData.studySessions.length > 0
        ) {
          const sessionsResult =
            await bulkOperationsService.bulkCreateStudySessions(
              userId,
              processedData.studySessions
            );

          if (sessionsResult.success && sessionsResult.data) {
            migrationResult.migratedItems.studySessions =
              sessionsResult.data.insertedCount;
        ...
        if (
          processedData.studyStatistics &&
          processedData.studyStatistics.length > 0
        ) {
      ...
      for (const stats of batch) {
        try {
          ...
          const existingResult =
            await studyStatisticsService.getStudyStatisticsById(
              stats.statsId,
              syncSession.userId
            );
      ...
      case 'study_sessions':
        await studySessionsService.updateSessionProgress(
          conflict.documentId,
          {
            ...
            currentPerformance: conflict.localVersion.performance as Partial<{
              overallScore: number;
              improvement: number;
              focusAreas: string[];
            }>,
  ...
  private async mergeVersions(conflict: SyncConflict): Promise<void> {
    ...
    const localTime = new Date(
      (conflict.localVersion.updatedAt || conflict.localVersion.createdAt) as
        | string
        | number
        | Date
    ).getTime();
    const remoteTime = new Date(
      (conflict.remoteVersion.updatedAt || conflict.remoteVersion.createdAt) as
        | string
        | number
        | Date
    ).getTime();
   ...
   */
  private async applyMergedVersion(
    conflict: SyncConflict,
    mergedData: Record<string, unknown>
  ): Promise<void> {
    switch (conflict.collection) {
      ...
      case 'study_sessions':
        await studySessionsService.updateSessionProgress(
          conflict.documentId,
          {
            ...
            currentPerformance: mergedData.performance as Partial<{
              overallScore: number;
              improvement: number;
              focusAreas: string[];
            }>,
   ...
   */
  private async getPendingSyncCounts(
    userId: string,
    since?: Date
  ): Promise<{
    flashcards: number;
  ...
  private async transformMigrationData(
    ...
    if (transformed.flashcards) {
      transformed.flashcards = (
        transformed.flashcards as Record<string, unknown>[]
    ...
    if (transformed.studySessions) {
      transformed.studySessions = (
        transformed.studySessions as Record<string, unknown>[]
    ...
    if (transformed.studyStatistics) {
      transformed.studyStatistics = (
        transformed.studyStatistics as Record<string, unknown>[]
      ).map((stats: Record<string, unknown>) => ({
        ...stats,
        userId,
   ...
   */
  private async validateMigrationData(
