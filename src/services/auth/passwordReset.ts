...
export function createInitiatePasswordResetHandler({ usersService, tokenManager }: PasswordResetDependencies) {
  return (resetData: PasswordResetData) =>
    safeAsync(async () => {
      ...
      const updateResult = await usersService.updateUser(user.userId, {
        ...user,
        authentication: {
          ...user.authentication,
      ...
      if (!updateResult.success) {
        throw new DatabaseError('Failed to initiate password reset', 'PASSWORD_RESET_FAILED', 'password_reset', 'users');
      }
...
export function createConfirmPasswordResetHandler({ usersService, tokenManager }: PasswordResetDependencies) {
  return (resetConfirmData: PasswordResetConfirmData) =>
    safeAsync(async () => {
      ...
      if (resetConfirmData.newPassword !== resetConfirmData.confirmPassword) {
        throw new ValidationError('Passwords do not match', 'password_reset_confirm', 'users');
      }

      const decoded = tokenManager.verifyPasswordResetToken(resetConfirmData.token);
      const userResult = await usersService.getUserById(decoded.userId);

      if (!userResult.success || !userResult.data) {
        throw new ValidationError('Invalid or expired reset token', 'password_reset_confirm', 'users');
      }

      const user = userResult.data;
      const tokenMatches = user.authentication.passwordResetToken === resetConfirmData.token;
      const tokenValid = user.authentication.passwordResetExpires && user.authentication.passwordResetExpires > new Date();
      ...
      if (!updateResult.success) {
        throw new DatabaseError('Failed to update password', 'PASSWORD_UPDATE_FAILED', 'password_reset_confirm', 'users');
      }

      SecurityAuditor.logSecurityEvent('PASSWORD_CHANGED', { userId: user.userId }, 'medium');
