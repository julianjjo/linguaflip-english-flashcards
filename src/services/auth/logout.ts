  ...
  return (userId: string, refreshToken: string) =>
    safeAsync(async () => {
      ...
      if (!userResult.success || !userResult.data) {
        throw new ValidationError('User not found', 'logout', 'users', 'userId', userId);
      }
      ...
      const updateResult = await usersService.updateUser(userId, {
        ...user,
        authentication: {
          ...user.authentication,
      ...
      if (!updateResult.success) {
        throw new DatabaseError('Failed to logout user', 'LOGOUT_FAILED', 'logout', 'users');
      }
