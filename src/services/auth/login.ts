...
export function createLoginHandler({ usersService, tokenManager, accountSecurity }: LoginDependencies) {
    ...
    safeAsync(async () => {
      ...
      try {
        ...
        if (!userResult.success || !userResult.data) {
          throw new ValidationError('Invalid email or password', 'login', 'users');
        }
      ...
      const isPasswordValid = await verifyPassword(loginData.password, user.authentication.password);

      if (!isPasswordValid) {
        await accountSecurity.incrementLoginAttempts(user.userId, loginData.ipAddress);

        if (user.security.loginAttempts + 1 >= AUTH_CONFIG.maxLoginAttempts) {
          await accountSecurity.lockAccount(user.userId, AUTH_CONFIG.lockoutDuration);
          throw new PermissionError(
