...
export const useAudioSystem = (): UseAudioSystemReturn => {
  ...
  const synthesizeFallbackAudio = async (
    text: string
    ...
    try {
      ...
      for (let i = 0; i < frameCount; i++) {
        ...
        if (word) {
          ...
          channelData[i] =
            Math.sin(time * (baseFreq + modulation) * 2 * Math.PI) * 0.1;
        }
      }
  ...
  const playAudioData = useCallback(
    async (audioData: Uint8Array): Promise<void> => {
  ...
  const speakWithGemini = useCallback(
    async (
      ...
      try {
        ...
        // Cache the audio
        await audioCacheService.current.storeAudio(
          text,
          voice,
          audioDataArray,
          mimeType,
  ...
  const preloadPronunciation = useCallback(
    async (text: string): Promise<void> => {
      ...
      if (settings.provider === 'gemini') {
        ...
        const cached = await audioCacheService.current.hasAudio(
          text,
