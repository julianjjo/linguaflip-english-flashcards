...
test.describe('Flujos principales adicionales de LinguaFlip', () => {
  test('un usuario puede completar el registro y es redirigido al dashboard', async ({ page }) => {
    ...
    await page.getByLabel('Nombre de usuario (opcional)').fill(registrationUser.username);
    await page.getByLabel('Contraseña *').fill(registrationUser.password);
    await page.getByLabel('Confirmar contraseña *').fill(registrationUser.password);
    ...
    await expect(successAlert).toContainText('Cuenta creada exitosamente. Redirigiendo...');
  ...
  test('el dashboard muestra estadísticas provenientes de la API y permite navegar a gestión de datos', async ({ page }) => {
    const statsPayload = {
      success: true,
      data: {
        totalCards: 180,
    ...
    await page.addInitScript(() => {
      ...
      window.localStorage.setItem('linguaflip_refresh_token', 'dashboard-refresh');
      window.localStorage.setItem(
        'linguaflip_user',
        JSON.stringify({ id: 'user_test_dashboard', email: 'dashboard@example.com' })
      );
    });
    ...
    await expect(page.getByRole('heading', { name: 'Dashboard de Progreso' })).toBeVisible();
    ...
    await expect(page.getByRole('link', { name: 'Gestionar Datos' })).toBeVisible();
    ...
    await expect(page.getByRole('heading', { name: 'Gestión de Datos' })).toBeVisible();
  });

  test('la página de progreso muestra las tarjetas de resumen y el acceso directo a nuevas sesiones', async ({ page }) => {
    await page.goto('/progress');

    await expect(page.getByRole('heading', { name: 'Progreso de Estudio' })).toBeVisible();
    ...
    const newSessionLink = page.getByRole('link', { name: 'Nueva Sesión de Estudio' });
    await expect(newSessionLink).toHaveAttribute('href', '/study');
  });

  test('los accesos rápidos de ayuda desplazan a la sección correspondiente', async ({ page }) => {
    await page.goto('/help');

    const faqSection = page.getByRole('heading', { name: 'Preguntas Frecuentes' });
  ...
  test('guardar cambios en configuración muestra un indicador temporal', async ({ page }) => {
    await page.goto('/settings');

    await expect(page.getByText('Settings component loading...')).toBeVisible();
